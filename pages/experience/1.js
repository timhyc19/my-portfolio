// pages/experience/1.js
import React from 'react';

const WorkExperience1 = () => {
  return (
    <div className="min-h-screen">
      <div className="px-10 md:px-20 lg:px-40 mt-20">
        <h1 className="text-2xl font-medium">Ecobee</h1>
        <p className="text-gray-600 text-xl">Software Engineer Intern | Summer 2023</p>
        <p className="text-gray-600 text-xl">Oustanding Rating</p>

        <h2 className="mt-10 text-xl">Summary</h2>
        <div className="max-w-4xl"> {/* Limit the width here */}
          <p className="text-xl">
            My initial work at ecobee was assisting with a new smart home doorbell camera.
            One of the products' features was a push notification to the users phone regarding any
            person, vehicle, and package detections. I contributed to the ML inference pipeline,
            which reported f1-scores of different models results based on a "golden set" of
            video images. Apart from following SOLID software principles, I was exposed to 
            technologies such as Python Multithreading, Kubernetes, and Docker.
          </p>

          <p className="text-xl mt-10">
          I then transitioned more into full stack development; creating a image annotation application,
          which allowed us to create different golden sets more efficiently. I built endpoints in Flask to
          retrieve and update image data in a GCP MySQL database, and wrote the database schema / code using 
          object relational mapping and object oriented principles. 
          </p>

          <p className="text-xl mt-10">
            I concluded my term by diving into infrastructure and technical debt work. I learned how to use Terraform,
            to maintain proper infrastructure of our GCP services, and also created new services like an Airflow Composer
            to host all our dags. I also developed the CI/CD procedure using Github Workflows and Actions, for the airlow repo, and 
            implemented new operators to run the pipelines more efficiently. 
            
          </p>
        </div>

        {/* You can add more details or sections as needed */}
      </div>
    </div>
  );
};

export default WorkExperience1;
